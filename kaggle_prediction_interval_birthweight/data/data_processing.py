...
class DataProcessor:
    ...
    def __init__(
        ...
        standardization_parameters: Dict
            Precomputed columnwise means and the decorrelation matrix, if already computed. None
            during training, but at test time these values should be passed in.
    ...
    def _transform_and_subset(self, df: pd.DataFrame) -> np.ndarray:
        """
        Transform features and drop columns.
        ...
        -------
        np.ndarray
            The array of features transformed and columns dropped.
        """
        ...
        categorical_features = list(set(keepers) - set(numeric_features))
        ...
        return np.hstack([x_one_hot, x_numeric])
    ...
    def __call__(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        X = self._transform_and_subset(df)
